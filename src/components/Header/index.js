...
function Header(props) {
  ...
  const { srollToLichChieu, srollToCumRap, srollToTinTuc, srollToUngDung } =
    props;
  ...
  const handleClickLichChieu = () => {
    if (match.path === "/home/detail/:movieId") {
      backToHome();
    } else {
      return srollToLichChieu();
    }
  };
  const handleClickCumRap = () => {
    if (match.path === "/home/detail/:movieId") {
      backToHome();
    } else {
      return srollToCumRap();
    }
  };
  const handleClickTinTuc = () => {
    if (match.path === "/home/detail/:movieId") {
      ...
      return srollToTinTuc();
    }
  };
  const handleClickUngDung = () => {
    if (match.path === "/home/detail/:movieId") {
      backToHome();
    } else {
      return srollToUngDung();
    }
  };
  ...
  const toggleDrawerClose = useCallback((event) => {
    if (
      event &&
      event.type === "keydown" &&
    ...
    <div className="header">
      <div className="header__container">
            ...
            <>
              <button onClick={handleClick}>
                <div className="avatar">
                  <Avatar>
                    {user ? user.hoTen.slice(0, 1).toUpperCase() : null}
                  </Avatar>
                </div>
                <div className="name">{user.taiKhoan}</div>
              </button>
              <Menu
                anchorEl={anchorEl}
              ...
              >
                <MenuItem>
                  <div className="link">
                    <Link to="/home/profile">Thông tin tài khoản</Link>
                  </div>
                  {loginUser.maLoaiNguoiDung === "QuanTri" ? (
                    <div className="link">
                      <Link to="/dashboard">Trang quản lý/Admin</Link>
                    </div>
                  ) : null}

                  <button onClick={handleLogout}>
                    Đăng xuất <i className="fas fa-sign-in-alt"></i>
                  </button>
                </MenuItem>
              </Menu>
            </>
        ...
        <div className="hamburger__icon">
          <MenuOpenIcon onClick={toggleDrawerOpen} />
          <SwipeableDrawer
            id="hamburger__icon--content"
            anchor="right"
            open={anchor}
          ...
          >
            <div className="container">
              {user ? (
                <div className="userHambur__logout">
                  <div
                    onClick={handleClick}
                    className="userHambur__logout--content"
                  >
                    <div className="userHambur__logout--avatar">
                      <Avatar>
                        {user ? user.hoTen.slice(0, 1).toUpperCase() : null}
                      </Avatar>
                    </div>
                    <div className="userHambur__logout--name">
                      {user.taiKhoan}
                    </div>
                  </div>
                  <button className="userHambur__logout--button">
                    <CloseIcon onClick={toggleDrawerClose} />
                  </button>
                </div>
              ) : (
                <div className="userHambur--login">
                  <Link
                    to="/home/log-in"
                    className="userHambur__login--content"
                  >
                    <div className="userHambur__login--avatar">
                      <Avatar></Avatar>
                    </div>
                    <div className="userHambur__login--name">LOGIN</div>
                  </Link>
                  <button className="userHambur__login--button">
                    <CloseIcon onClick={toggleDrawerClose} />
                  </button>
                </div>
              )}
              <hr className="hr" />
              {user ? (
                <Accordion className="info__hambur">
                  <AccordionSummary
                    className="info__hambur--title"
                    expandIcon={<ExpandMoreIcon />}
                  >
                    Thông tin tài khoản
                  </AccordionSummary>
                  <AccordionDetails className="info__hambur--content">
                    <div className="info__hambur--acount">
                      <Link to="/home/profile">Thông tin tài khoản</Link>
                    </div>
                    <button
                      onClick={handleLogout}
                      className="info__hambur--logout"
                    >
                      Đăng xuất <i className="fas fa-sign-in-alt"></i>
                    </button>
                  </AccordionDetails>
